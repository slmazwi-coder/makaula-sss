...
type UploadedFile = { key: string; label: string; fileName: string; mimeType: string; dataUrl: string };
type Application = {
  id: string; firstName: string; lastName: string; dob: string; gender: string;
  grade: string; year: string; studentNumber: string; guardianName: string;
  guardianRelationship: string; guardianPhone: string; guardianEmail: string;
  address: string; locality: string; previousSchool: string; lastGradeCompleted: string;
  medicalInfo: string; applicationType: string; status: string; submittedDate: string;
...
const inp =
  'border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#DAA520]/40 focus:border-[#0D1B4C] transition w-full bg-white';
...
const SectionHeading = ({ icon, title }: { icon: React.ReactNode; title: string }) => (
  <div className="flex items-center gap-2 pb-2 border-b border-gray-200 mb-4">
    <span className="text-[#0D1B4C]">{icon}</span>
  ...
  <div className="flex items-center gap-2">
    <div
      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black border-2 transition-all
        ${done   ? 'bg-[#DAA520] border-[#0D1B4C] text-white'
        : active ? 'bg-white border-white text-[#0D1B4C]'
  ...
  if (submitted) {
    return (
      <div className="py-20 flex items-center justify-center min-h-[60vh]">
        ...
        >
          <div className="w-20 h-20 bg-[#FFF3CC] text-[#0D1B4C] rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle size={48} />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Application Submitted!</h2>
          <p className="text-gray-600 mb-8">
            Thank you for applying to Makaula Senior Secondary School. We have received your application and will be in contact shortly.
          </p>
    ...
    <div className="py-12 sm:py-16 bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
          ...
          <div className="bg-[#DAA520] px-8 py-7 text-white">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
              <div>
                <h2 className="text-2xl font-bold">Application for Admission to School</h2>
                <p className="text-white/70 text-sm mt-1">
                  Makaula Senior Secondary School &nbsp;·&nbsp; Makaula, Eastern Cape
                </p>
              </div>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="text-sm font-semibold text-[#0D1B4C] underline underline-offset-2"
                      >
                          ...
                          return (
                            <div
                              ...
                              className={`relative flex items-center gap-3 p-3 rounded-xl border-2 transition
                                ${file
                                  ? 'border-[#0D1B4C] bg-[#EEF1F8]'
                                  : field.required
                                    ? 'border-dashed border-red-300 bg-red-50/40'
                                    : 'border-dashed border-gray-300 bg-gray-50'}`}
                            >
                              <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${file ? 'bg-[#DAA520] text-white' : 'bg-gray-200 text-gray-400'}`}>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {file && <div className="text-xs text-[#0D1B4C] truncate mt-0.5">{file.name}</div>}
                                {!file && <div className="text-xs text-gray-400 mt-0.5">No file chosen</div>}
                              </div>
                              <label className="shrink-0 text-xs font-bold text-[#0D1B4C] cursor-pointer hover:underline">
                                {file ? 'Change' : 'Upload'}
                                <input
                                  type="file"
                                  className="hidden"
                    ...
                    <section className="bg-gray-50 border border-gray-200 rounded-2xl p-5">
                      ...
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <input
                          ...
                          className="mt-0.5 w-4 h-4 accent-[#DAA520] cursor-pointer"
                        />
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  {step < 3 ? (
                    <button
                      type="button"
                      onClick={goNext}
                      className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#DAA520] text-white text-sm font-bold hover:bg-[#B89200]/90 transition shadow"
                    >
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="inline-flex items-center gap-2 px-7 py-2.5 rounded-xl bg-[#DAA520] text-white text-sm font-bold hover:bg-[#B89200]/90 disabled:opacity-60 disabled:cursor-not-allowed transition shadow"
                    >
                      {submitting ? (
